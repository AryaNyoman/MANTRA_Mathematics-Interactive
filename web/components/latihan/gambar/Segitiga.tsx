...
export default function Segitiga({
  ...
  return (
    <svg viewBox={`0 0 ${LEBAR} ${TINGGI}`} preserveAspectRatio="xMidYMid meet" role="img"
      ...
      <line x1={ax} y1={ay} x2={bx} y2={ay} stroke={WARNA.samping} strokeWidth={2.4} />
      <line x1={bx} y1={ay} x2={bx} y2={cy} stroke={WARNA.depan} strokeWidth={2.4} />
      <line x1={ax} y1={ay} x2={bx} y2={cy} stroke={WARNA.miring} strokeWidth={2.4} />
      ...
      <text x={(ax + bx) / 2} y={ay + 20} fontSize={13} textAnchor="middle" fill={WARNA.samping} fontFamily={MONO}>{samping}</text>
      <text x={bx + 10} y={(ay + cy) / 2 + 5} fontSize={13} fill={WARNA.depan} fontFamily={MONO}>{depan}</text>
      <text x={(ax + bx) / 2 - 12} y={(ay + cy) / 2 - 10} fontSize={13} textAnchor="end" fill={WARNA.miring} fontFamily={MONO}>{miring}</text>
    </svg>
  )
}
