...
export type Jendela = {
   ...
   * (NaikPangkat, LuasYangTumbuh) memakai papan yang lebih pendek supaya
   * keduanya muat bertumpuk di kolom alat bersama kendalinya (ARYA 18 Sep
   * 2026). Semua pemetaan (keLayar, keMatematika, Bidang) membacanya.
   */
  tinggi?: number
}

/**
 * Tinggi papan untuk widget BERPAPAN DUA (f di atas, F di bawah). Papan
 * tunggal setinggi VH; dua papan setinggi VH bertumpuk mendorong kendali
 * keluar layar, jadi keduanya dibuat lebih pendek: pada tinggi kolom yang
 * sama gambarnya boleh lebih lebar, dan hurufnya tetap terbaca. Angkanya
 * sama dengan milik widget Turunan (PAPAN_DUA di widget/turunan/koordinat.ts).
 */
