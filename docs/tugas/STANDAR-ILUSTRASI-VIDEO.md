...
6. **Huruf: Constantia untuk kata, LaTeX untuk angka dan rumus.** `teks()` untuk
   kalimat dan label ("dayung 3 km"), `rumus()` untuk angka berdiri sendiri, nilai
   hidup, dan rumus. Keterangan lewat `sinema.keterangan` (satu baris, mengganti
   dirinya, beralas krem). Paling banyak dua blok teks yang harus dibaca sekaligus.
   Satuan di rumus: `\mathrm{km}`, BUKAN `\text{km}` (dibuang diam-diam).
