...
- [ ] 1.1 `web/app/globals.css`: ganti blok token gerak (baris 96 sampai 101) dengan
...
- [ ] 1.2 `web/react-canary.d.ts`: `/// <reference types="react/canary" />`.
- [ ] 1.3 `web/components/mantra/Panggung.tsx` (client): `<ViewTransition enter="panggung-masuk" exit="panggung-keluar" default="none">`.
...
- [ ] 1.4 `web/app/layout.tsx`: `<ArahRute />`, `<Nav />` sekali; hapus `<Nav>` dari
...
- [ ] 1.5 `Nav.tsx`: tidak `return null` saat fokus (`data-fokus` + `inert`);
...
- [ ] 1.6 `PetaMateri.tsx` (`kisi-bab`) dan `DaftarLatihan.tsx`: `data-bertahap`
      dengan `--n`, lepas `MunculSaatGulir` di daftar itu (satu sumber gerak);
      `[data-transisi]` mematikan bertahap.
- [ ] 1.7 Elemen bersama: `judul-bab-<slug>` pada h3 kartu bab dan h1 HalamanTopik,
      ...
      hasilnya jelek.
- [ ] 1.8 `app/topik/[slug]/page.tsx`: `data-isi-asli` pada pembungkus isi
      (`isi-ganti` 160 ms) supaya kerangka → isi tidak melompat.
- [ ] 1.9 Cek: Beranda → Peta Materi → belajar → kembali (arah), tab kanan/kiri,
...
- [ ] 2.1 `web/components/mantra/Jendela.tsx` (`<dialog>`, showModal, Esc,
...
- [ ] 2.2 `ArenaLatihan.tsx`: tiga jendela → `<Jendela>`; `useHitung` (900 ms,
      sessionStorage `matra:hitung-<halaman>`) untuk skor, `bar-besar`,
      `latihan-persen`; `.skor-angka .garis`; `.lencana-raih` bertahap `--n`.
- [ ] 2.3 Umpan balik: `.opsi-mantra .tanda` `tanda-tumbuh`; `.kabar-periksa`;
...
- [ ] 2.4 `Kuis.tsx`, `Latihan.tsx`: `.opsi .tanda` dan pembahasan `naik-umpan`
      dengan bahasa yang sama (tanpa goyang).
- [ ] 2.5 Cek: Periksa benar/salah, Berikutnya 5× cepat, Lihat skor 2× cepat,
